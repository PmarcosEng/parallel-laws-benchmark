...
/* ─────────────────────────────────────────────────────────────
   KERNEL — Reduce Paralelo com redução em shared memory

   Cada bloco de 256 threads:
     1. Carrega 1 elemento: predicate 0/1 em sdata[tid]
     2. Árvore de redução: log2(256) = 8 passos em SHMEM (on-chip)
     3. Thread 0 faz atomicAdd com a soma do bloco

   Vantagens sobre kernel_busca_linear:
     - Sem array de saída grande (sem writes espalhados na VRAM)
     - Shared memory elimina acessos globais no reduce
     - Apenas 1 atomicAdd por bloco (vs 1 por match na busca linear)
     - Compute-bound em vez de memory-bound na fase de reduce
───────────────────────────────────────────────────────────── */
__global__ void kernel_reduce_count(
    const Event* __restrict__ dados,
    int   n,
    ...
    int*  __restrict__ d_result)
{
    ...
    /* cada thread: 1 se match, 0 caso contrário */
    sdata[tid] = (i < n && dados[i].valor >= vmin && dados[i].valor <= vmax) ? 1 : 0;
    __syncthreads();

    /* redução em árvore dentro do bloco — log2(blockDim.x) passos */
    ...
    /* thread 0 do bloco acumula no resultado global via atomicAdd */
    if (tid == 0)
        atomicAdd(d_result, sdata[0]);
}

/* ─────────────────────────────────────────────────────────────
   reduce_paralelo_cuda — pipeline completo

   Etapas:
     [ev0] ─ H2D transfer ─ [ev1] ─ kernel reduce ─ [ev2]
               t_h2d                  t_kernel
           └──────────── t_total ───────────────────┘
───────────────────────────────────────────────────────────── */
extern "C" SearchResult reduce_paralelo_cuda(
    Event *dados, int n, float vmin, float vmax,
    double *t_total_s, double *t_kernel_s)
{
    const int THREADS_POR_BLOCO = 256;
    const int N_BLOCOS = (n + THREADS_POR_BLOCO - 1) / THREADS_POR_BLOCO;
    const size_t SHMEM = (size_t)THREADS_POR_BLOCO * sizeof(int);
    ...
    /* fase 1: H2D — cópia dos eventos via PCIe */
    CUDA_CHECK(cudaEventRecord(ev0));
    ...
    /* fase 2: kernel com shared memory reduction */
    CUDA_CHECK(cudaEventRecord(ev2));
    kernel_reduce_count<<<N_BLOCOS, THREADS_POR_BLOCO, SHMEM>>>(
        d_dados, n, vmin, vmax, d_result);
    CUDA_CHECK(cudaEventRecord(ev3));
    CUDA_CHECK(cudaEventSynchronize(ev3));

    /* resultado: único inteiro — D2H barato */
    int count = 0;
    CUDA_CHECK(cudaMemcpy(&count, d_result, sizeof(int), cudaMemcpyDeviceToHost));
